...
def create_image(width: int, height: int, background_color: Color) -> Image:
    return Image(
        width, height, [[background_color for _ in range(width)] for _ in range(height)]
    )
...
def draw_image(image: Image):
    ...
    for y in range(height):
        for x in range(width):
            color = image.pixels[y][x]
            # Assuming the alpha channel is always 255 (fully opaque)
            numpy_image[y, x] = [color.red, color.green, color.blue, 255]

    h, w, d = numpy_image.shape
    numpy_image = np.ravel(np.uint8(np.reshape(numpy_image, (h * w * d, -1)))).tobytes()
    ...
    if pyCanv is not None:
        ...
        print(
            "Could not find div to append canvs to. There should be a div with 'python-canvas' id"
        )
...
#
# clear_screen(img, background_color)  # Clear the screen with the background color
