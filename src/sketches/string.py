...
class String:
    ...
    def update(self):
        ...
        for i in range(1,self.n-1):
            y = (self.physical_const  
                * (self.points[i-1] + self.points[i+1] - 2 * self.points[i])
                + 2 * self.points[i] - self.past_points[i]
                )
                #niave damping
...
def onMouseMove(event:MouseEvent):
    ...
    move_end(x,y)
def onMouseUp(event:MouseEvent):
